...
int main(int argc, char **argv) {
...
#ifdef __ANDROID__
	bListenLibusb = 0;
	DBG_LOG("Mencoba konversi termux ke port usb fd.\n");
	// handle
	if (xfd < 0)
		ERR_EXIT("Contoh: termux-usb -e \"./spd_dump --usb-fd\" /dev/bus/usb/xxx/xxx\n"
			"berjalan di android perlu menyediakan --usb-fd\n");

	if (libusb_wrap_sys_device(NULL, (intptr_t)xfd, &io->dev_handle))
		ERR_EXIT("libusb_wrap_sys_device keluar tanpa syarat!\n");

	curPort = libusb_get_device(io->dev_handle);
	if (libusb_get_device_descriptor(curPort, &desc))
		ERR_EXIT("libusb_get_device keluar tanpa syarat!");

	DBG_LOG("Vendor ID: %04x\nProduct ID: %04x\n", desc.idVendor, desc.idProduct);
	if (desc.idVendor != 0x1782 || desc.idProduct != 0x4d00) {
		ERR_EXIT("Sepertinya perangkat spek bukan perangkat spd!\n");
	}
	...
	for (i = 0; ; i++) {
		if (io->recv_buf[2] == BSL_REP_VER) {
			...
			else ERR_EXIT("perintah yang salah atau mode yang salah terdeteksi, reboot ponsel Anda dengan menekan POWER dan VOL_UP selama 7-10 detik.\n");
		}
		else {
			send_msg(io);
			recv_msg(io);
		...
		if (ret == BSL_REP_ACK || ret == BSL_REP_VER || ret == BSL_REP_VERIFY_ERROR) {
			if (ret == BSL_REP_VER) {
				if (fdl1_loaded == 1) {
					DBG_LOG("MENGECEK_BAUD FDL1\n");
					if (!memcmp(io->raw_buf + 4, "SPRD4", 5)) fdl2_executed = -1;
				}
				else {
					DBG_LOG("MENGECEK_BAUD BOOTROM\n");
			...
			if (stage != -1) ERR_EXIT("perintah yang salah atau mode yang salah terdeteksi, reboot ponsel Anda dengan menekan POWER dan VOL_UP selama 7-10 detik.\n");
	...
	while (1) {
		...
		if (!strcmp(str2[1], "sendloop")) {
			...
			if (fdl2_executed > 0) {
				DBG_LOG("FDL2 SUDAH DIEKSEKUSI, SKIP\n");
				...
				for (i = 0; ; i++) {
					...
					DBG_LOG("MENGECEK_BAUD GAGAL\n");
					if (i == 4) ERR_EXIT("perintah yang salah atau mode yang salah terdeteksi, reboot ponsel Anda dengan menekan POWER dan VOL_UP selama 7-10 detik.\n");
					usleep(500000);
				}
				DBG_LOG("MENGECEK_BAUD FDL1\n");
				...
				if (baudrate) {
					...
					if (!send_and_check(io)) {
						DBG_LOG("MENGUBAH_BAUD FDL1 to %d\n", baudrate);
			...
			if (fdl2_executed > 0) {
				...
				if (ret == BSL_REP_INCOMPATIBLE_PARTITION)
					...
					ERR_EXIT("respon tak terduga (0x%04x)\n", ret);
				DBG_LOG("EKSEKUSI FDL2\n");
				encode_msg(io, BSL_CMD_READ_FLASH_INFO, NULL, 0);
				send_msg(io);
				ret = recv_msg(io);
				if (ret) {
					ret = recv_type(io);
					if (ret != BSL_REP_READ_FLASH_INFO) DBG_LOG("respon tak terduga (0x%04x)\n", ret);
			...
			if (0 == fdl1_loaded && argcount > 2) {
				...
				if (fi == NULL) { DBG_LOG("%s tidak ditemukan\n", execfile); exec_addr = 0; }
				else fclose(fi);
			}
			...
			DBG_LOG("nand_id sekarang adalah 0x%x\n", nand_id);
			argc -= 2; argv += 2;
			...
			if (!send_and_check(io)) DBG_LOG("Erase Flash Selesai: 0x%08x\n", (int)addr);
			argc -= 3; argv += 3;
