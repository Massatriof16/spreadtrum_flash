...
int main(int argc, char **argv) {
	...
	for (i = 0; ; i++) {
		...
		if (ret == BSL_REP_ACK || ret == BSL_REP_VER || ret == BSL_REP_VERIFY_ERROR) {
			...
			if (stage != -1) ERR_EXIT("perintah salah atau mode salah terdeteksi, reboot your phone by pressing POWER and VOL_UP for 7-10 seconds.\n");
	...
	while (1) {
		...
		if (!strcmp(str2[1], "sendloop")) {
			...
			if (fdl2_executed > 0) {
				...
				if (gpt_failed != 1) {
					if (selected_ab == 2) DBG_LOG("perangkat menggunakan slot b\n");
					else if (selected_ab == 1) DBG_LOG("perangkat menggunakan a\n");
					else {
						DBG_LOG("perangkat tidak menggunakan VAB\n");
						if (Da_Info.bSupportRawData) {
...
rloop:
			...
			if (!io->part_count) { DBG_LOG("Partition table tidak tersedia\n"); argc -= 3; argv += 3; continue; }
			fn = str2[3];
			fi = fopen(fn, "r");
			if (fi == NULL) { DBG_LOG("File tidak ada.\n"); argc -= 3; argv += 3; continue; }
			else fclose(fi);
